    ...
    <div className="container-fluid p-4 text-white">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-white-50 small mb-0">
            Publish, edit, or configure front-page featured status parameters for media briefings.
          </p>
        </div>
        <Link to={ROUTES.ADMIN_NEWS_NEW} className="btn btn-primary fw-bold py-2">
      ...
      {news?.length === 0 ? (
        <div className="card bg-dark border-secondary p-5 text-center text-white-50">
          ...
          <p className="small">Get started by creating your first global media publication.</p>
        </div>
      ) : (
          ...
          <table className="table table-dark table-hover mb-0 align-middle">
            <thead>
              <tr className="border-bottom border-secondary text-white-50">
                ...
                <th scope="col" style={{ width: "140px" }}>Featured</th>
                <th scope="col" style={{ width: "140px" }}>Published Date</th>
            ...
            <tbody>
              {news?.map((item) => (
                <tr key={item.id} className="border-bottom border-secondary">
                  ...
                  <td>
                    <div className="fw-semibold text-white mb-0 text-truncate" style={{ maxWidth: "420px" }} title={item.title}>
                      {item.title}
                    </div>
                    {item.author && <span className="text-muted small d-block" style={{ fontSize: "0.75rem" }}>By {item.author}</span>}
                  </td>
                  <td>
                    <span className="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-25 text-uppercase" style={{ fontSize: "0.7rem", letterSpacing: "0.3px" }}>
                  ...
                  <td>
                    {item.isFeatured ? (
                      ...
                      <span className="text-white-50 small opacity-50">Standard</span>
                    )}
                  </td>
