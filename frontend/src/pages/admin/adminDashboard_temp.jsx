...
function AdminDashboard() {
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      const active = usersSnapshot.docs.filter(
        (doc) => doc.data().membershipStatus === "Active",
      );

      setActiveMembers(active.length);

      const pending = membershipsSnapshot.docs.filter(
  ...
  return (
    <div>
      ...
      <Link to="/admin/applications">Membership Applications</Link>
    </div>
  );
}
