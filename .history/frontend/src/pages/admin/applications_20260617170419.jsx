  ...
  return (
    <div>
      ...
      {applications.map((app) => (
        ...
        >
          <p>ID: {app.nationalId}</p>
          <p>County: {app.county}</p>
          <p>Vessel: {app.vesselType}</p>
          <p>Status: {app.status}</p>
          const confirmed = window.confirm( "Approve this member?" );
          if(!confirmed) return;
          <button
            disabled={app.status === "Active"}
            onClick={() => approveMembership(app)}
          >
            Approve
          </button>
          <button onClick={() => rejectApplication(app)}>Reject</button>
        </div>
      ))}
    </div>
  );
